      ...
      <div className="z-10 w-full max-w-4xl flex flex-col items-center animate-in fade-in zoom-in duration-700">
        <div className="mb-12 flex flex-col items-center text-center">
          ...
          <p className="text-xl text-muted-foreground max-w-2xl">
            Real-time patient safety intelligence from fragmented signals to actionable insights.
          </p>
        </div>

        <div className="grid md:grid-cols-2 gap-6 w-full max-w-2xl">
          <Card 
            className="glass-card p-8 cursor-pointer group flex flex-col items-center text-center hover:-translate-y-1 transition-transform"
            onClick={() => handleRoleSelect("Health Official")}
          >
            <div className="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
              <Building2 className="w-8 h-8 text-primary" />
            </div>
            <h3 className="text-xl font-semibold mb-2">Enter as Health Official</h3>
            <p className="text-sm text-muted-foreground">Access command center, cluster detection, and strategic analytics.</p>
          </Card>

          <Card 
            className="glass-card p-8 cursor-pointer group flex flex-col items-center text-center hover:-translate-y-1 transition-transform"
            onClick={() => handleRoleSelect("Field Worker")}
          >
            ...
            <p className="text-sm text-muted-foreground">Submit incident reports, track local alerts, and monitor community health.</p>
          </Card>
        </div>
      </div>
    </div>
